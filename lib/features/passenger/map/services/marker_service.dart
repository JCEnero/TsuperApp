...
class MarkerService {
  ...
  Future<BitmapDescriptor> getJeepneyMarker({
    ...
    final cacheKey = '${status.name}_${heading.toStringAsFixed(1)}_${size.toStringAsFixed(1)}';
  ...
  Future<BitmapDescriptor> _createJeepneyMarker({
    ...
    if (_jeepneyImage == null && !_imageLoadAttempted) {
      ...
      try {
        if (kDebugMode) {
          print('[MarkerService] Loading jeepney image from assets/markers/jeepney_navy.png');
        }
        final ByteData byteData = await rootBundle.load('assets/markers/jeepney_navy.png');
        final Uint8List pngBytes = byteData.buffer.asUint8List();
        final ui.Codec codec = await ui.instantiateImageCodec(pngBytes);
        final ui.FrameInfo frameInfo = await codec.getNextFrame();
        _jeepneyImage = frameInfo.image;
        if (kDebugMode) {
          print('[MarkerService] Jeepney image loaded successfully: ${_jeepneyImage!.width}x${_jeepneyImage!.height}');
        }
      } catch (e) {
        if (kDebugMode) {
    ...
    if (_jeepneyImage != null) {
      ...
      canvas.drawImageRect(
        _jeepneyImage!,
        Rect.fromLTWH(0, 0, _jeepneyImage!.width.toDouble(), _jeepneyImage!.height.toDouble()),
